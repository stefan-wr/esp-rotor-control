...
namespace Rotor {
    ...
    void Messenger::setLastRotationMsg(const bool &with_angle) {
        if (rotor_ptr != NULL) {
            ...
            if (!with_angle && rotor_ptr->is_auto_rotating) {
                doc["target"] = round(rotor_ptr->auto_rotation_target * 100.0f) / 100.0f;
            }
            ...
            if (with_angle) {
                doc["adc_v"] = round(rotor_ptr->rotor.last_adc_volts
                                     ...
                                     * 1000.0f) / 1000.0f;
                doc["angle"] = round(rotor_ptr->rotor.last_angle * 100.0f) / 100.0f;
            }
    ...
    void Messenger::sendCalibration() {
        ...
        doc["a1"] = round(rotor_ptr->rotor.calibration.a1 * 10000.0f) / 10000.0f;
        doc["u1"] = round(rotor_ptr->rotor.calibration.u1 * 10000.0f) / 10000.0f;
        doc["a2"] = round(rotor_ptr->rotor.calibration.a2 * 10000.0f) / 10000.0f;
        doc["u2"] = round(rotor_ptr->rotor.calibration.u2 * 10000.0f) / 10000.0f;
